...
glm::vec3 pointLightPositions[] = {
	glm::vec3(-5.0f,0.0f, 3.5f),//Cubo Fogata
	glm::vec3(4.5f,2.0f, 1.5f),// Cubo Lampara
	glm::vec3(2.0f,9.0f,  0.9f), //Cubo Luna
...
int main()
{
	...
	GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT, "Practica_9 Alvarez Lugo Victor Manuel", nullptr, nullptr);
	...
	Model Luna((char*)"Models/Luna.obj");//Se carga modelo de la luna
	Model Lampara((char*)"Models/Linterna.obj");//Se carga modelo de la lampara
	Model Fogata((char*)"Models/campfire.obj");//Se carga el modelo de la fogata
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// Directional light
		glUniform3f(glGetUniformLocation(lightingShader.Program, "dirLight.direction"), 0.2f, -1.0f, -0.3f); //Aqui se configura "hacia donde apunta" la luz  
		glUniform3f(glGetUniformLocation(lightingShader.Program, "dirLight.ambient"), 0.05f, 0.05f, 0.01f);
		glUniform3f(glGetUniformLocation(lightingShader.Program, "dirLight.diffuse"), 0.15f, 0.15f, 0.25f);
		glUniform3f(glGetUniformLocation(lightingShader.Program, "dirLight.specular"), 0.05f, 0.05f, 0.1f);


		// Point light 1 (Fogata)
		glm::vec3 lightColor;
		...
		glUniform3f(glGetUniformLocation(lightingShader.Program, "pointLights[0].specular"), 0.5f, 0.2f, 0.2f);
		glUniform1f(glGetUniformLocation(lightingShader.Program, "pointLights[0].constant"), 1.0f); //Son necesarios para el calculo de la atenuacion
		...
		glUniform3f(glGetUniformLocation(lightingShader.Program, "pointLights[1].ambient"), 0.3f, 0.3f, 0.1f);
		...
		glUniform3f(glGetUniformLocation(lightingShader.Program, "pointLights[2].ambient"), 0.1f, 0.1f, 0.2f);
		glUniform3f(glGetUniformLocation(lightingShader.Program, "pointLights[2].diffuse"), 0.2f, 0.2f, 0.4f);
		...
		model = glm::translate(model, glm::vec3(22.0f, 0.0f, 7.0f));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		...
		glUniformMatrix4fv(glGetUniformLocation(lightingShader.Program, "model"), 1, GL_FALSE, glm::value_ptr(modelBat));
		bat.Draw(lightingShader);// Dibujamos el modelo
		...
		glUniformMatrix4fv(glGetUniformLocation(lightingShader.Program, "model"), 1, GL_FALSE, glm::value_ptr(modelCastle));
		castle.Draw(lightingShader);//Dibujamos el modelo
		...
		glUniformMatrix4fv(glGetUniformLocation(lightingShader.Program, "model"), 1, GL_FALSE, glm::value_ptr(modelWolf));
		wolf.Draw(lightingShader);//Dibujamos el modelo
		...
		glUniformMatrix4fv(glGetUniformLocation(lightingShader.Program, "model"), 1, GL_FALSE, glm::value_ptr(modelGhost));
		ghost.Draw(lightingShader);//Dibujamos el modelo
		...
		glUniformMatrix4fv(glGetUniformLocation(lightingShader.Program, "model"), 1, GL_FALSE, glm::value_ptr(modelVampire));
		vampire.Draw(lightingShader);//Se dibuja el model
		glBindVertexArray(0);
...
void DoMovement()
{
	...
	if (keys[GLFW_KEY_T])
	{
		pointLightPositions[0].x += 0.0001f;
	}
	if (keys[GLFW_KEY_G])
	{
		pointLightPositions[0].x -= 0.0001f;
